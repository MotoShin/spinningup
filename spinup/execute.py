...
class Execute:
    ...
    def add_with_backends(self, algo_list):
    ...
    def friendly_err(self, err_msg):
        # add whitespace to error message to make it more readable
        return '\n\n' + err_msg + '\n\n'


    def parse_and_execute_grid_search(self, cmd, args):
        ...
        for i, arg in enumerate(args.split()):
            assert i > 0 or '--' in arg, \
                self.friendly_err("You didn't specify a first flag.")
            if '--' in arg:
                arg_key = arg.lstrip('-')
                arg_dict[arg_key] = []
            else:
        ...
        for k in RUN_KEYS:
            if k in arg_dict:
                val = arg_dict[k]
                assert len(val) == 1, \
                    self.friendly_err("You can only provide one value for %s."%k)
                run_kwargs[k] = val[0]
                del arg_dict[k]
        ...
        if 'exp_name' in arg_dict:
            assert len(arg_dict['exp_name']) == 1, \
                self.friendly_err("You can only provide one value for exp_name.")
        ...
        if 'num_cpu' in run_kwargs and not(run_kwargs['num_cpu'] == 1):
            assert cmd in self.add_with_backends(MPI_COMPATIBLE_ALGOS), \
                self.friendly_err("This algorithm can't be run with num_cpu > 1.")
        ...
        assert 'env_name' in arg_dict, \
            self.friendly_err("You did not give a value for --env_name! Add one and try again.")
    ...
    def run(self):
        ...
        if self.cmd in valid_help:
        ...
        elif self.cmd in valid_utils:
            # Execute the correct utility file.
            runfile = osp.join(osp.abspath(osp.dirname(__file__)), 'utils', self.cmd +'.py')
            command = [sys.executable if sys.executable else 'python', runfile] + self.args.split()
            print(command)
            ...
            self.parse_and_execute_grid_search(self.cmd, self.args)
